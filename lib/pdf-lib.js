...
var ZTOC_PDF = (function () {
	...
	// FlateDecode via DecompressionStream, présent aussi bien dans Gecko que dans
	// Node. Certains fichiers annoncent du zlib mais contiennent du deflate brut :
	// on réessaie sans en-tête plutôt que d'abandonner.
	async function inflate(bytes) {
		let tries = ["deflate", "deflate-raw"];
		let lastErr = null;
		for (let fmt of tries) {
			try {
				let ds = new DecompressionStream(fmt);
				let stream = new Blob([bytes]).stream().pipeThrough(ds);
				let out = new Uint8Array(await new Response(stream).arrayBuffer());
				if (out.length) return out;
			}
			catch (e) { lastErr = e; }
		}
		// Dernier recours : flux tronqué. On relit octet par octet ce qui passe.
		throw new Error("FlateDecode a échoué : " + (lastErr && lastErr.message));
	}
