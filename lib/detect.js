...
var ZTOC_DETECT = (function () {
	...
	function runningKey(text) {
		return normKey(text)
			.replace(/\d+/g, " ")
			// Les pages liminaires d'un livre sont numérotées en chiffres romains :
			// « Preface / ix » et « Preface / xi » doivent donner la même clé.
	...
	// Un renvoi de sommaire imprimé se termine par un numéro de page — avec des
	// points de conduite, ou séparé par un simple espace, en chiffres arabes ou
	// romains. N'exiger que les points de conduite laisse passer la plupart des
	// ouvrages, dont les entrées ressemblent à « An Introduction 1 ».
	const TOC_LINE = /(?:\.{3,}\s*|\s)(?:[ivxlcdm]{1,7}|\d{1,4})\s*$/i;
	const TOC_TITLE = /^(?:contents|table of contents|sommaire|table des mati[èe]res|index)$/i;
	...
	function findTocPages(pages) {
		...
		for (let p of pages) {
			...
			// Une page intitulée « CONTENTS » est un sommaire, même si l'analyse
			// de ses lignes ne suffit pas à le prouver.
			let titled = p.lines.some(l => TOC_TITLE.test(l.text.trim()));

			if (lines.length >= 4) {
				let refs = 0;
				for (let l of lines) if (TOC_LINE.test(l.text)) refs++;
				if (refs >= Math.max(3, lines.length * 0.3)) { out.add(p.pageIndex); continue; }
				// Sur une page explicitement intitulée, on se contente d'un
				// faisceau plus mince : les entrées peuvent courir sur deux lignes.
				if (titled && refs >= 2) { out.add(p.pageIndex); continue; }
			}
			if (titled) out.add(p.pageIndex);
		}
		return out;
	}

	// Un intitulé qui revient à l'identique sur plusieurs pages n'est pas une
	// section : « This page intentionally left blank », mentions d'éditeur, etc.
	// Les sections usuelles (« Introduction ») sont exemptées : elles se répètent
	// légitimement d'un chapitre à l'autre.
	function findRepeatedLines(pages) {
		let count = new Map();
		for (let p of pages) {
			let seen = new Set();
			for (let l of p.lines) {
				let k = normKey(l.text);
				if (!k || k.length < 4 || seen.has(k)) continue;
				seen.add(k);
				count.set(k, (count.get(k) || 0) + 1);
			}
		}
		let out = new Set();
		for (let [k, n] of count) if (n >= 3) out.add(k);
		return out;
	}
	...
	function rejectLine(text) {
		...
		// En-tête courant portant son folio (« Preface / ix », « Chapitre 2 | 47 ») :
		// la barre de séparation suivie d'un numéro seul ne se rencontre pas dans
		// un vrai titre de section.
		if (/\s[\/|·]\s*(?:[ivxlcdm]{1,7}|\d{1,4})\s*$/i.test(text)) return true;
	...
	function detect(doc, options) {
		...
		let repeated = findRepeatedLines(pages);
		...
		for (let p of pages) {
			...
			for (let i = 0; i < lines.length; i++) {
				...
				if (repeated.has(normKey(text)) && !KEYWORDS.test(text)) continue;
	...
	return {
		...
		_internals: { numbering: numbering, normKey: normKey, findRepeatedLines: findRepeatedLines, styleCensus: styleCensus, findRunningHeads: findRunningHeads, findTocPages: findTocPages }
	};
})();
